...
class CanvasZH {
	...
	_draw () {
		...
		// draw the lines
		for (let i = 0; i < this.point; i++) {
			const cirA = this.circleArr[i];
			this._drawCircle(cirA);
			for (let j = 0; j < this.point - i; j++) {
				const cirB = this.circleArr[j];
				const distanceX = Math.abs(cirB.x - cirA.x);
				const distanceY = Math.abs(cirB.y - cirB.y);
